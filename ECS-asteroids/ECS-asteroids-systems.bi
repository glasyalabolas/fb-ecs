...
function getCollisionNormal(N as Vec2, v1 as Vec2, v2 as Vec2) as Vec2
  ...
  '' of the collision.
  var tangent = N.turnedRight().normalize(), vRel = v1 - v2
...
function resolveCollision(bc1 as BoundingCircle, bc2 as BoundingCircle, vN as Vec2, vel1 as Vec2, vel2 as Vec2) as Vec2
...
function collided(a1 as BoundingCircle, a2 as BoundingCircle, vel1 as Vec2, vel2 as Vec2) as Vec2  
...
type CollidableSystem extends ECSSystem
  ...
  private:
    ...
    '' Caching the id of the optional component allows for direct reference
    ...
    for each a as ECSEntity in other
      ...
      if a <> prnt[b].id andAlso bbb.overlapsWith(abb) then
        ...
        if h[a].current <= 0.0f then
          ECS.raiseEvent(EV_GAME_ENTITY_SHOT, GameEntityDestroyedEventArgs(a, prnt[b].id, myEntities, myComponents), @this)
        end if
      end if 
    next
  next
...
sub HealthSystem.process(dt as double = 0.0d)
  ...
  for each e as ECSEntity in destroyed
    ECS.raiseEvent(EV_GAME_ENTITY_DESTROYED, GameEntityDestroyedEventArgs(e, -1, myEntities, myComponents), @this)
...
type AsteroidDestroyedSystem extends ECSSystem
  ...
  private:
    declare static sub event_gameEntityDestroyed(as any ptr, as GameEntityDestroyedEventArgs, as AsteroidDestroyedSystem ptr)
...
type ScoreSystem extends ECSSystem
  ...
  private:
    declare static sub event_gameEntityDestroyed(as any ptr, as GameEntityDestroyedEventArgs, as ScoreSystem ptr)
