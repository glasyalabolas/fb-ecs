...
namespace __FBG_NS__
  type MouseInput
    public:
      ...
      declare property startX() as long
      declare property startY() as long
      declare property horizontalWheel() as long
      declare property verticalWheel() as long
      ...
      declare function drag( as long ) as boolean
      declare function drop( as long ) as boolean
  ...
  property MouseInput.X() as long
    return( _x )
  end property
  
  property MouseInput.Y() as long
    return( _y )
  end property
  
  property MouseInput.deltaX() as long
    return( _dx )
  end property
  
  property MouseInput.deltaY() as long
    return( _dy )
  end property
  
  property MouseInput.startX() as long
    return( _sx )
  end property
  
  property MouseInput.startY() as long
    return( _sy )
  end property
  
  property MouseInput.horizontalWheel() as long
    return( _hWheel )
  end property
  
  property MouseInput.verticalWheel() as long
    return( _vWheel )
  end property
      ...
      select case as const( ev->type )
        case Fb.EVENT_MOUSE_MOVE
          /'
            ...
            as a ushort (0..65535).
          '/
          _x = *cast( short ptr, @ev->x ) : _y = *cast( short ptr, @ev->y )
          
          if( _pressed ) then
            _dx = _x - _sx : _dy = _y - _sy
            
            _dragging = true : _dropped = false
          end if
        ...
        case Fb.EVENT_MOUSE_BUTTON_RELEASE
          ...
          _pressed = false
          _dropped = cbool( _dx <> 0 andAlso _dy <> 0 )
  ...
  '/
  function MouseInput.pressed( aButton as long ) as boolean
    mutexLock( _mutex )
  ...
  /'
    ...
    above.
  '/
  function MouseInput.released( aButton as long ) as boolean
    mutexLock( _mutex )
  ...
  /'
    ...
    in milliseconds.
  '/
  function MouseInput.held( aButton as long, interval as double = 0.0d ) as boolean
    mutexLock( _mutex )
      ...
      if( cbool( _state( aButton ) and ButtonState.Held ) ) then
        ...
        if( cbool( interval > 0.0d ) ) then
          if( not cbool( _state( aButton ) and ButtonState.HeldInitialized ) ) then
            ...
            dim as double elapsed = ( timer() - _heldStartTime( aButton ) ) * 1000.0d
            
            if( elapsed >= interval ) then
              isHeld = false
              
              _state( aButton ) = _state( aButton ) and not ButtonState.Held
            end if
          end if
        end if
      end if
  ...
  /'
    ...
    'held()'.
  '/
  function MouseInput.repeated( aButton as long, interval as double = 0.0d ) as boolean
    mutexLock( _mutex )
      ...
      if( cbool( _state( aButton ) and ButtonState.Repeated ) ) then
        if( cbool( interval > 0.0d ) ) then
          if( not cbool( _state( aButton ) and ButtonState.RepeatedInitialized ) ) then
            ...
            dim as double elapsed = ( timer() - _repeatedStartTime( aButton ) ) * 1000.0d
            
            if( elapsed >= interval ) then
              isRepeated = true
              
              _state( aButton ) = _state( aButton ) and not ButtonState.RepeatedInitialized
            end if
          end if
        else
          isRepeated = true
  ...
  function MouseInput.drag( aButton as long ) as boolean
    return( held( aButton ) andAlso _dragging )
  end function
  
  function MouseInput.drop( aButton as long ) as boolean
