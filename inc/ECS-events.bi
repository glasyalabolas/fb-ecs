...
#define toHandler(_fp_)(cptr(ECSEventHandler, @_fp_))
...
type ECSEvents
  public:
    ...
    declare function registerListener(as ECSEvent, as ECSEventHandler, as any ptr = 0) as boolean
    declare function unregisterListener(as ECSEvent, as ECSEventHandler, as any ptr = 0) as boolean
...
destructor ECSEvents()
  for i as integer = 0 to _size - 1
    do while _bucket(i).listeners.count > 0
      delete(cptr(ECSEventRegister ptr, _bucket(i).listeners.removeLast()))
    loop
  next
...
function ECSEvents.find(eID as ECSEvent) as ECSEventTableEntry ptr
  ...
  do while current <> -1
    ...
    current = _bucket(current).next
  loop
  
  return NULL
end function

function ECSEvents.registerListener(eID as ECSEvent, handler as ECSEventHandler, receiver as any ptr = 0) as boolean
  var e = find(eID)
...
function ECSEvents.unregisterListener(eID as ECSEvent, handler as ECSEventHandler, receiver as any ptr = 0) as boolean
  var e = find(eID)
  ...
  if e <> 0 then
    ...
    do while n <> 0
      var entry = cptr(ECSEventRegister ptr, n->item)
      
      if entry->handler = handler andAlso entry->receiver = receiver then
        delete(cptr(ECSEventRegister ptr, e->listeners.remove(n)))
        return true
      end if
...
sub ECSEvents.raise(eID as ECSEvent, p as ECSEventArgs, sender as any ptr = 0)
  ...
  if e <> 0 then
    ...
    for i as integer = 0 to e->listeners.count - 1
      cptr(ECSEventRegister ptr, n->item)->handler(sender, p, cptr(ECSEventRegister ptr, n->item)->receiver)
